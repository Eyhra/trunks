...
/**     https://ronovelty.com/      **/
/*************************************/
#define MAX_REPUTATION 4 // defined in data\contentdata\repute\reputeinfodata.bson
...
struct s_unit_common_data {
	struct s_ucd_aura {
		...
		bool hidden = false;	// Do you need to hide the aura
	} aura;
};
...
struct s_storage {
	...
	union { // Max for inventory, storage, cart, and guild storage are 818 each without changing this struct and struct item [2016/08/14]
		...
		struct item items_cart_bkp[MAX_CART];
	} u;
};
