...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if ((p->autovend_id != cp->autovend_id) ||
		(p->autovend_mapid != cp->autovend_mapid) ||
		(p->autovend_posx != cp->autovend_posx) ||
		(p->autovend_posy != cp->autovend_posy) ||
		(p->autovend_check != cp->autovend_check))
	{
		if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `autovend_id`='%d', `autovend_mapid`='%d', `autovend_posx`='%d', `autovend_posy`='%d', `autovend_check`='%d'"
			" WHERE  `account_id`='%d' AND `char_id` = '%d'",
			schema_config.char_db, p->autovend_id, p->autovend_mapid, p->autovend_posx, p->autovend_posy, p->autovend_check, p->account_id, p->char_id))
		{
			Sql_ShowDebug(sql_handle);
			errors++;
		}
		else
			strcat(save_status, " autovend");
	}
...
int char_memitemdata_to_sql(const struct item items[], int max, int id, enum storage_type tableswitch, uint8 stor_id) {
	...
	switch (tableswitch) {
		...
		case TABLE_CART_BKP:
			printname = "Cart_bkp";
			tablename = schema_config.cart_bkp_db;
			selectoption = "char_id";
			break;
		case TABLE_STORAGE:
...
int char_mmo_chars_fromsql(struct char_session_data* sd, uint8* buf, uint8* count ) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`hotkey_rowshift2`,`autovend_mapid`,`autovend_posx`,`autovend_posy`,`autovend_id`, `autovend_check`, "
	...
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 46, SQLDT_SHORT,	&p.autovend_mapid, 0, NULL, NULL)
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 47, SQLDT_SHORT,	&p.autovend_posx, 0, NULL, NULL)
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 48, SQLDT_SHORT,	&p.autovend_posy, 0, NULL, NULL)
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 49, SQLDT_INT,	&p.autovend_id, 0, NULL, NULL)
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 50, SQLDT_INT,	&p.autovend_check, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 51, SQLDT_UINT,   &p.max_ap, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 52, SQLDT_UINT,   &p.ap, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 53, SQLDT_UINT,   &p.trait_point, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 54, SQLDT_SHORT,  &p.pow, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 55, SQLDT_SHORT,  &p.sta, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 56, SQLDT_SHORT,  &p.wis, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 57, SQLDT_SHORT,  &p.spl, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 58, SQLDT_SHORT,  &p.con, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 59, SQLDT_SHORT,  &p.crt, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 60, SQLDT_UINT16, &p.inventory_slots, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	if( SQL_ERROR == SqlStmt_Prepare(stmt, "SELECT "
		...
		"`autovend_mapid`,`autovend_posx`,`autovend_posy`,`autovend_id`,`autovend_check`,"
		"`max_ap`,`ap`,`trait_point`,`pow`,`sta`,`wis`,`spl`,`con`,`crt`,"
		"`inventory_slots`"
	...
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 63, SQLDT_SHORT,	&p->autovend_mapid, 0, NULL, NULL)
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 64, SQLDT_SHORT,	&p->autovend_posx, 0, NULL, NULL)
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 65, SQLDT_SHORT,	&p->autovend_posy, 0, NULL, NULL)
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 66, SQLDT_INT,	&p->autovend_id, 0, NULL, NULL)
	||  SQL_ERROR == SqlStmt_BindColumn(stmt, 67, SQLDT_INT,	&p->autovend_check, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 68, SQLDT_UINT,   &p->max_ap, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 69, SQLDT_UINT,   &p->ap, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 70, SQLDT_UINT,   &p->trait_point, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 71, SQLDT_SHORT,  &p->pow, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 72, SQLDT_SHORT,  &p->sta, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 73, SQLDT_SHORT,  &p->wis, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 74, SQLDT_SHORT,  &p->spl, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 75, SQLDT_SHORT,  &p->con, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 76, SQLDT_SHORT,  &p->crt, 0, NULL, NULL)
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 77, SQLDT_UINT16, &p->inventory_slots, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
...
bool char_checkdb(void){
	int i;
	const char* sqltable[] = {
		schema_config.char_db, schema_config.hotkey_db, schema_config.scdata_db, schema_config.cart_db,schema_config.cart_bkp_db,
	...
	//checking cart_bkp_db
	if (SQL_ERROR == Sql_Query(sql_handle, "SELECT  `id`,`char_id`,`nameid`,`amount`,`equip`,`identify`,`refine`,"
		"`attribute`,`card0`,`card1`,`card2`,`card3`,`option_id0`,`option_val0`,`option_parm0`,`option_id1`,`option_val1`,`option_parm1`,`option_id2`,`option_val2`,`option_parm2`,`option_id3`,`option_val3`,`option_parm3`,`option_id4`,`option_val4`,`option_parm4`,`expire_time`,`bound`,`unique_id`,`enchantgrade`"
		" FROM `%s` LIMIT 1;", schema_config.cart_bkp_db)) {
		Sql_ShowDebug(sql_handle);
		return false;
	}
...
void char_sql_config_read(const char* cfgName) {
	...
	while(fgets(line, sizeof(line), fp)) {
		...
		else if (!strcmpi(w1, "cart_bkp_db"))
			safestrncpy(schema_config.cart_bkp_db, w2, sizeof(schema_config.cart_bkp_db));
...
//	schema_config.db_use_sqldbs;
	...
	safestrncpy(schema_config.cart_bkp_db, "cart_bkp_inventory", sizeof(schema_config.cart_bkp_db));
