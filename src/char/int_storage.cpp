 ...
 * @return 0 if success, or error count
 */
int cart_bkp_tosql(uint32 char_id, struct s_storage* p)
{
	return char_memitemdata_to_sql(p->u.items_cart_bkp, MAX_CART, char_id, TABLE_CART_BKP, p->stor_id);
}
 ...
 * @return True if success, False if failed
 */
bool cart_bkp_fromsql(uint32 char_id, struct s_storage* p)
{
	return char_memitemdata_from_sql(p, MAX_CART, char_id, TABLE_CART_BKP, p->stor_id);
}
...
bool mapif_parse_StorageLoad(int fd) {
	...
	switch (type) {
		...
		case TABLE_CART_BKP:  res = cart_bkp_fromsql(cid, &stor);  break;
...
bool mapif_parse_StorageSave(int fd) {
	...
	switch(type){
		...
		case TABLE_CART_BKP: cart_bkp_tosql(cid, &stor); break;
		default: return false;
	}
