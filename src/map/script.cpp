...
BUILDIN_FUNC(menu)
{
	...
	{// Cancel was pressed
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"cancel.wav",0);
	}
	...
	{// goto target label
		...
		if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
		clif_soundeffect(sd,&sd->bl,"option.wav",0);	
		}
		st->state = GOTO;
	}
	return SCRIPT_CMD_SUCCESS;
...
BUILDIN_FUNC(skill)
{
	...
	* -------------------------------------------------------
	* caso o flag for ADDSKILL_TEMP ou 1, aciona a função
	* que é responsavel por guardar a skill pra ser removida
	* mais tarde quando o equipamento for removido
	*--------------------------------------------------------*/
	if (flag == ADDSKILL_TEMP)
		pc_check_temp_skill(sd, id);
...
#define getunitdata_sub(idx__,var__) setd_sub_num(st,sd,name,(idx__),(var__),data->ref)
	...
	switch(bl->type) {
		case BL_MOB:
			...
			getunitdata_sub(UMOB_DAMAGETAKEN, md->damagetaken); // By: KyoSasuke#5401
			getunitdata_sub(UMOB_DAMAGETAKEN_DB, md->db->damagetaken); // By: KyoSasuke#5401
...
BUILDIN_FUNC(setunitdata)
{
	...
	case BL_MOB:
		...
		switch (type) {
			...
			case UMOB_DAMAGETAKEN: md->damagetaken = cap_value(value, -1, UINT16_MAX); break; // By: KyoSasuke#5401
...
/*************************************/
/**
 * Get reputation point of a player
 * getreputpoint(<id>)
 * returns -1 on fail
 */
BUILDIN_FUNC(getreputpoint) {
	int reputID;
	...
	reputID = script_getnum(st, 2);

	if(reputID <= 0 || reputID > MAX_REPUTATION){
		ShowError("Reputation ID must be between %d and %d.",1,MAX_REPUTATION);
		return SCRIPT_CMD_FAILURE;
	}

	script_pushint( st, sd->reputation[reputID-1].points );

	return SCRIPT_CMD_SUCCESS;
}

/**
 * Set reputation point of a player
 * setreputpoint(<reputID>,<points>)
 * returns -1 on fail
 */
BUILDIN_FUNC(setreputpoint) {
	int points=0, reputID;
	...
	sd->reputation[reputID-1].points = sd->reputation[reputID-1].points + points;
	clif_reputation(sd);
...
/**
 * open reputation UI on a player with selected groupID and (option) colored reputation ID
 * openreputui(<groupID>,<reputID>)
 * returns -1 on fail
 */
BUILDIN_FUNC(openreputui) {
	int reputID, groupID;
	...
	groupID = script_getnum(st, 2);
	reputID = script_getnum(st, 3);

	if(groupID < 0 || groupID > 255){
		ShowError("Group reputation ID must be between %d and %d.",0,255);
		return -1;
	}
	if(reputID < 0 || reputID > MAX_REPUTATION){
		ShowError("Reputation ID must be between %d (color disabled) and %d.",0,MAX_REPUTATION);
		return -1;
	}

	clif_reputation_open(sd, groupID, reputID);
 ...
 * Usage: aura <aura number>{,<character number>};
...
/* ===========================================================
 * Command: unitaura
 * Description: Used to adjust the aura combination of seven units (but only BL_PC will be persisted)
 * Usage: unitaura <unit number>,<aura number>;
 * Return: 1 on success, 0 on failure
 * -----------------------------------------------------------*/
...
// <--- [zBuffer] List of unit control commands
	...
	BUILDIN_DEF(getreputpoint, "i"),
	BUILDIN_DEF(setreputpoint, "ii"),
	BUILDIN_DEF(openreputui, "ii"),
