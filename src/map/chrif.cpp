 ...
 *  CSAVE_CART_BKP: character used autovend
 */
int chrif_save(struct map_session_data *sd, int flag) {
	...
	if (flag & CSAVE_CART_BKP) {
		intif_storage_bkp_save(sd);
		return 0;
	}
	...
	if ( (flag&CSAVE_QUITTING) && (sd->state.active || sd->autovend.active)) { //Store player data which is quitting
	...
	for(int i=0;i<MAX_REPUTATION;i++)
		pc_setaccountreg(sd, reference_uid( add_str( REPUTATION_VAR ), i ), sd->reputation[i].points);
...
void chrif_parse_ack_vipActive(int fd) {
	...
	if ((flag&0x2)) //isgm
		...
		if((flag&0x1)) { //isvip
			...
			sc_start(NULL, &sd->bl, SC_VIP, 100, 1, (vip_time-time(NULL)) * 1000);
