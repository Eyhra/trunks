...
char cart_bkp_table[32] = "cart_bkp_inventory";
...
char cart_items_table[32] = "cart_inventory";
char cart_bkp_items_table[32] = "cart_bkp_inventory";
...
void map_flags_init(void){
	for (int i = 0; i < map_num; i++) {
		...
		mapdata->skill_itemcost.clear();	// Skill Item Cost
		map_free_questinfo(mapdata);
...
void map_data_copy(struct map_data *dst_map, struct map_data *src_map) {
	...
	dst_map->skill_itemcost.insert(src_map->skill_itemcost.begin(), src_map->skill_itemcost.end());	// Skill Item Cost
...
// Skill Item Cost
void map_skill_itemcost_add(struct map_data *mapd, s_skill_itemcost skill_itemcost) {
	nullpo_retv(mapd);
	nullpo_retv(skill_itemcost);

	if (mapd->skill_itemcost.find(skill_itemcost.skill_id) != mapd->skill_itemcost.end()) // Entry exists
		mapd->skill_itemcost[skill_itemcost.skill_id] = skill_itemcost;
	else // Update previous entry
		mapd->skill_itemcost.insert({ skill_itemcost.skill_id, skill_itemcost });
}
...
bool map_setmapflag_sub(int16 m, enum e_mapflag mapflag, bool status, union u_mapflag_args *args)
{
	...
	switch(mapflag) {
		...
		case MF_SKILL_ITEMCOST:
			if (!status)
				mapdata->skill_itemcost.clear();
			else {
				nullpo_retr(false, args);

				map_skill_itemcost_add(mapdata, args->skill_itemcost);
			}
			mapdata->flag[mapflag] = status;
			break;
		case MF_ATK_RATE:
