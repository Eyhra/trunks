...
static char log_picktype2char(e_log_pick_type type)
{
	switch( type )
	{
		...
		case LOG_TYPE_AUTOVEND:			return '#';	 // (W)AUTOVEND
	}
...
void log_pick(int id, int16 m, e_log_pick_type type, int amount, struct item* itm)
{
	...
	if( log_config.sql_logs )
	{
		...
		if (type == LOG_TYPE_AUTOVEND) {
			StringBuf_Printf(&buf, ") VALUES(NOW(),'%u','%c','%u','%d','%d','%s','%" PRIu64 "','%d','%d'",
				id, 24, itm->nameid, amount, itm->refine, map_getmapdata(m)->name[0] ? map_getmapdata(m)->name : "", itm->unique_id, itm->bound, itm->enchantgrade);
		}
		else {
			StringBuf_Printf(&buf, ") VALUES(NOW(),'%u','%c','%u','%d','%d','%s','%" PRIu64 "','%d','%d'",
