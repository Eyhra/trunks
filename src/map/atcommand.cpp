		...
		if (!((pc_has_permission(pl_sd, PC_PERM_HIDE_SESSION) || pc_isinvisible(pl_sd)) && pc_get_group_level(pl_sd) > level)) { // you can look only lower or same level
			...
			switch (display_type) {
				...
				case 3: {
					...
					if (pl_sd->autovend.account_id) // [Autovend]: Auto Vending
...
ACMD_FUNC(reput)
{
	int reput=0, reputID, ret=-1;
	nullpo_retr(-1, sd);

	if (!message || !*message || sscanf(message, "%11d %11d", &reputID, &reput) < 1){
		clif_displaymessage(fd, "Reputation change failed.");
		clif_displaymessage(fd, "Use @reput <reputID> <reputationPoints>.");
		return -1;
	}

	if(reputID <= 0 || reputID > MAX_REPUTATION){
		sprintf(atcmd_output, "Reputation ID must be between %d and %d.",1,MAX_REPUTATION);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}

	//sd->reputation[reputID-1].points = sd->reputation[reputID-1].points + reput;
	//clif_reputation(sd);

	//sprintf(atcmd_output, "New reputation point is %d.",sd->reputation[reputID-1].points);
	//clif_displaymessage(fd, atcmd_output);
	sd->reputation[reputID-1].points = sd->reputation[reputID-1].points + reput;
    clif_reputation(sd);

    sprintf(atcmd_output, "New reputation point is %d.",static_cast<int>(sd->reputation[reputID-1].points));
    clif_displaymessage(fd, atcmd_output);
...
ACMD_FUNC(reload) {
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		...
		aura_reload();
		clif_displaymessage(fd, msg_txt(sd, 1605)); // Aura database has been reloaded.
	}
...
ACMD_FUNC(antimob)
{
	...
	nullpo_retr(-1, sd);
	if (map_getmapflag(sd->bl.m, MF_GVG)) {
		clif_displaymessage(fd, msg_txt(sd,1770)); // O comando @antimob está desativado em castelos.
		return -1;
	}
	if (DIFF_TICK(sd->ks_floodprotect_tick, tick) <= 0) {
		if (sd->state.antimob) {
			sd->state.antimob = 0;
			sd->ks_floodprotect_tick = tick + 60000;
			clif_displaymessage(fd, msg_txt(sd,1771)); // [ Proteção PvP Anti Mob Inativa ] - Você pode usar o próximo @antimob depois de 1 minuto
		}
		else {
			sd->state.antimob = 1;
			clif_displaymessage(fd, msg_txt(sd,1772)); // [ Proteção PvP Anti Mob Ativa ]
		}
		return 0;
	}
	else {
		clif_displaymessage(fd, msg_txt(sd,1773)); // Comando falhou, @antimob ainda em Cool Down.
		return -1;
	}
}
...
* Made by Vengence
* Make it work for latest [Cookie]
*------------------------------------------*/
...
if (!message || !*message) {
clif_displaymessage(fd, "Use: @jumptosell (<+refine> )(<item_id>)(<[card_id]>) or @jumptosell <item_name>");
return -1;
}
...
if (map_getmapflag(sd->bl.m, MF_TOWN)) {
	if (sd->status.zeny < 1000) {
		clif_messagecolor( &sd->bl, color_table[COLOR_RED], "You don't have enough zeny.", false, SELF );
		return 0;
	}
	sd->status.zeny -= 1000;
	pc_setpos(sd, pl_sd->mapindex, pl_sd->bl.x, pl_sd->bl.y, CLR_TELEPORT);
	...
	clif_messagecolor( &sd->bl, color_table[COLOR_RED], "You can only use this command in towns.", false, SELF );
	return 0;
}
}
}
...
ACMD_FUNC(mobinfo)
{
	...
	int count;
	int i, k;
	int64 rateExp = pc_readglobalreg(sd, add_str("irateExp"));
	if(!rateExp) rateExp = 1;
	...
	for (k = 0; k < count; k++) {
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			int droprate = mob_getdroprate( &sd->bl, mob, mob->dropitem[i].rate, drop_modifier, item_data );
			//int droprate = mob_getdroprate(&sd->bl, mob, mob->dropitem[i].rate, drop_modifier, item_data);
...
ACMD_FUNC(whodrops)
{
	...
	int i,j, count = 1;
	int64 rateDrop = pc_readglobalreg(sd, add_str("irateDrop"));
	...
	for (i = 0; i < count; i++) {
		...
		if (item_data->mob[0].chance == 0) {
			...
			for (j=0; j < MAX_SEARCH && item_data->mob[j].chance > 0; j++)
			{
				...
				if(rateDrop && (item_data->type != IT_CARD || (mob->get_bosstype() != BOSSTYPE_MVP && mob->get_bosstype() != BOSSTYPE_MINIBOSS)))
					dropchance += (int)(dropchance * rateDrop / 100);	
...
ACMD_FUNC(rates)
{
	...
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1298), // Experience rates: Base %.2fx / Job %.2fx
		(rateExp * (battle_config.base_exp_rate + (pc_isvip(sd) ? (battle_config.vip_base_exp_increase * battle_config.base_exp_rate) / 100 : 0))) / 100.,
		(rateExp * (battle_config.job_exp_rate + (pc_isvip(sd) ? (battle_config.vip_job_exp_increase * battle_config.job_exp_rate) / 100 : 0))) / 100.);
	...
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1299), // Normal Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common + ((rateDrop * battle_config.item_rate_common) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_common) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal + ((rateDrop * battle_config.item_rate_heal) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal) / 100 : 0)) / 100.,
		(battle_config.item_rate_use + ((rateDrop * battle_config.item_rate_use) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip + ((rateDrop * battle_config.item_rate_equip) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_equip) / 100 : 0)) / 100.,
		(battle_config.item_rate_card + ((rateDrop * battle_config.item_rate_card) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_card) / 100 : 0)) / 100.);
	clif_displaymessage(fd, buf);
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1300), // Boss Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common_boss + ((rateDrop * battle_config.item_rate_common) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_common_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal_boss + ((rateDrop * battle_config.item_rate_heal_boss) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_use_boss + ((rateDrop * battle_config.item_rate_use_boss) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use_boss) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip_boss + ((rateDrop * battle_config.item_rate_equip_boss) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_boss) / 100 : 0)) / 100.,
	...
	snprintf(buf, CHAT_SIZE_MAX, msg_txt(sd,1024), // MVP Drop Rates: Common %.2fx / Healing %.2fx / Usable %.2fx / Equipment %.2fx / Card %.2fx
		(battle_config.item_rate_common_mvp + ((rateDrop * battle_config.item_rate_common_mvp) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_common_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_heal_mvp + ((rateDrop * battle_config.item_rate_heal_mvp) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_heal_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_use_mvp + ((rateDrop * battle_config.item_rate_use_mvp) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_use_mvp) / 100 : 0)) / 100.,
		(battle_config.item_rate_equip_mvp + ((rateDrop * battle_config.item_rate_equip_mvp) / 100) + (pc_isvip(sd) ? (battle_config.vip_drop_increase * battle_config.item_rate_equip_mvp) / 100 : 0)) / 100.,
...
ACMD_FUNC(itemlist)
{
	...
	if( strcmp(parent_cmd, "storagelist") == 0 ) {
		...
		size = MAX_CART;
	} else if( strcmp(parent_cmd, "cartbkplist") == 0 ) {
		location = "cart_bkp";
		items = sd->cart_bkp.u.items_cart_bkp;
		size = MAX_CART;
...
ACMD_FUNC(eledef) {
	...
	struct {
		...
		{ "   [ %d ] Resistencia Neutro", 0 },
		{ "   [ %d ] Resistencia Agua", 0 },
		{ "   [ %d ] Resistencia Terra", 0 },
		{ "   [ %d ] Resistencia Fogo", 0 },
		{ "   [ %d ] Resistencia Vento", 0 },
		{ "   [ %d ] Resistencia Veneno", 0 },
		{ "   [ %d ] Resistencia Sagrado", 0 },
		{ "   [ %d ] Resistencia Sombio", 0 },
		{ "   [ %d ] Resistencia Fantasma", 0 },
		{ "   [ %d ] Resistencia MortoVivo", 0 },
//		{ "   [ %d ] Resist to all", 0 },
		{ NULL, 0 }
	};
	memset(output, '\0', sizeof(output));
	clif_displaymessage(sd->fd, "========= Resistencia Elemental =========");
...
ACMD_FUNC(globaldef) {
	...
	struct {
		...
		{ NULL, 0 },
		{ "   [ %d ] Resistencia tamanho pequeno", 0 }, // 1
		{ "   [ %d ] Resistencia tamanho medio", 0 },
		{ "   [ %d ] Resistencia tamanho grande", 0 },

		{ "   [ %d ] Resistencia Normal class", 0 },
		{ "   [ %d ] Resistencia Boss class", 0 },
		{ "   [ %d ] Resistencia Guardian class", 0 },
		{ "   [ %d ] Resistencia Battlefield class", 0 }, // 7

		{ "   [ %d ] Resistencia Raca Formless", 0 },
		{ "   [ %d ] Resistencia Raca Undead", 0 },
		{ "   [ %d ] Resistencia Raca Bruto", 0 },
		{ "   [ %d ] Resistencia Raca Planta", 0 },
		{ "   [ %d ] Resistencia Raca Inseto", 0 },
		{ "   [ %d ] Resistencia Raca Peixe", 0 },
		{ "   [ %d ] Resistencia Raca Demon", 0 },
		{ "   [ %d ] Resistencia Raca Demi-Human", 0 },
		{ "   [ %d ] Resistencia Raca Anjo", 0 },
		{ "   [ %d ] Resistencia Raca Dragao", 0 },
		{ "   [ %d ] Resistencia Raca Player", 0 }, // 18

		{ "   [ %d ] Resistencia do Goblin", 0 },
		{ "   [ %d ] Resistencia do Kobold", 0 },
		{ "   [ %d ] Resistencia do Orc", 0 },
		{ "   [ %d ] Resistencia do Golem", 0 },
		{ "   [ %d ] Resistencia do Guardian", 0 },
		{ "   [ %d ] Resistencia do Ninja", 0 },
		{ "   [ %d ] Resistencia do GVG", 0 },
		{ "   [ %d ] Resistencia do Battlefield", 0 },
		{ "   [ %d ] Resistencia do Treasure", 0 },
		{ "   [ %d ] Resistencia do Biolab", 0 },
		{ "   [ %d ] Resistencia do Manuk", 0 },
		{ "   [ %d ] Resistencia do Splendide", 0 },
		{ "   [ %d ] Resistencia do Scaraba", 0 }, // 31

		{ "   [ %d ] Resistencia ao elemento Neutro", 0 },
		{ "   [ %d ] Resistencia ao elemento Agua", 0 },
		{ "   [ %d ] Resistencia ao elemento Terra", 0 },
		{ "   [ %d ] Resistencia ao elemento Fogo", 0 },
		{ "   [ %d ] Resistencia ao elemento Vento", 0 },
		{ "   [ %d ] Resistencia ao elemento Veneno", 0 },
		{ "   [ %d ] Resistencia ao elemento Sagrado", 0 },
		{ "   [ %d ] Resistencia ao elemento Sombrio", 0 },
		{ "   [ %d ] Resistencia ao elemento Fantasma", 0 },
		{ "   [ %d ] Resistencia ao elemento MortoVivo", 0 }, // 41

		{ "   [ %d ] Defesa de elemento Neutro", 0 },
		{ "   [ %d ] Defesa de elemento Agua", 0 },
		{ "   [ %d ] Defesa de elemento Terra", 0 },
		{ "   [ %d ] Defesa de elemento Fogo", 0 },
		{ "   [ %d ] Defesa de elemento Vento", 0 },
		{ "   [ %d ] Defesa de elemento Veneno", 0 },
		{ "   [ %d ] Defesa de elemento Sagrado", 0 },
		{ "   [ %d ] Defesa de elemento Sombrio", 0 },
		{ "   [ %d ] Defesa de elemento Fantasma", 0 },
		{ "   [ %d ] Defesa de elemento MortoVivo", 0 }, // 51

		{ "   [ %d ] Resistencia a danos de longo alcance", 0 },
		{ "   [ %d ] Resistencia a danos corpo a corpo", 0 }, // 53
		{ "   [ %d ] Imunidade a armas", 0 },
		{ "   [ %d ] Imunidade magica", 0 }, 
		{ "   [ %d ] Imunidade diversa", 0 }, // 56
		{ NULL, 0 }
	};
	...
	if( count == 0 ){
		sprintf(resis_state, "Você não tem nenhuma resistencia.");
		clif_messagecolor(&sd->bl, color_table[COLOR_RED], resis_state, false, SELF);
	} else {
		sprintf(resis_state, "======== Resistencia Global ========");
...
ACMD_FUNC(autovend)
{
	...
	if (!sd->state.vending) { //check if player is vending
		clif_displaymessage(fd, msg_txt(sd, 1753)); // Você deve ter uma loja aberta para usar o @autovend.
		return -1;
	}
 ...
 * Update: NeoWolfer
 */
ACMD_FUNC(autovendclose)
{
	nullpo_retr(-1, sd);

	if (!battle_config.autovend_enable) {
		clif_displaymessage(fd, msg_txt(sd, 1750)); // Auto venda não está ativado.
		return -1;
	}

	vending_close_autovend(sd);
	return 0;
}
...
ACMD_FUNC(equipoff) {
	...
	if (map_getmapflag(sd->bl.m, MF_NOEQUIP_OFF) || map_getmapflag(sd->bl.m, MF_EQUIP_HIDE)) {
		clif_displaymessage(fd, msg_txt(sd, 1805)); // Voce nao pode usar @equipoff or @equipon neste mapa!
		return -1;
	}
	...
	if (strcmp(parent_cmd, "equipoff") == 0) {
		if ((sscanf(message, "%s", &type)) < 1) {
			clif_displaymessage(fd, msg_txt(sd, 1803)); // Syntax: @equipoff <type>
			clif_displaymessage(fd, msg_txt(sd, 1804)); // Type: all - all players | others - other players
			return -1;
		}

		if ((strncmp(type, "todos", 3) == 0))
			value += 2;

		if ((strncmp(type, "outros", 3) == 0))
			value += 1;
		...
		if ((strncmp(message, "outros", 3) == 0) || (value&2)) {
		...
		clif_displaymessage(fd,  msg_txt(sd, 1802)); // Use @equipon to disable the system.
		clif_displaymessage(fd, msg_txt(sd, 1800)); // EquipOff activated!

	} else if (strcmp(parent_cmd, "equipon") == 0) {
		sd->state.equiphide = 0;
		clif_refresh(sd);
		clif_status_change(&sd->bl, EFST_EQUIP_HIDE, 0, INVALID_TIMER, 0, 0, 0);
		clif_displaymessage(fd, msg_txt(sd, 1801)); // EquipOff disabled!
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_TOP, sd->status.head_top, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_MID, sd->status.head_mid, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_HEAD_BOTTOM, sd->status.head_bottom, SELF);
		clif_refreshlook(&sd->bl, sd->bl.id, LOOK_ROBE, sd->status.robe, SELF);
...
/**
 * Command: aura
 * Description: Activate the specified aura combination
 * Usage: @aura
 **/
ACMD_FUNC(aura) {
	uint32 aura_id = 0;
	...
	if (!message || !*message || sscanf(message, "%11d", &aura_id) < 1) {
		clif_displaymessage(fd, msg_txt(sd, 1600));	// How to use: @aura <aura number, if set to 0 to cancel the aura>
		clif_displaymessage(fd, msg_txt(sd, 1601));	// Aura numbers are defined in the aura combination database in db/aura_db.yml, see the comments at the top of the database for more information.
		return -1;
	}
	...
	if (aura_id && !aura_search(aura_id)) {
		clif_displaymessage(fd, msg_txt(sd, 1604));	// Sorry, the specified aura number is invalid, please check and re-enter.
		return -1;
	}
	...
	if (aura_id != 0) {
		clif_displaymessage(fd, msg_txt(sd, 1602));	// The specified aura effect is activated.
	}
	else {
		clif_displaymessage(fd, msg_txt(sd, 1603));	// Aura effect turned off.
	}
...
#include "../custom/atcommand_def.inc"
		...
		ACMD_DEF2("cartbkplist", itemlist),
		...
		ACMD_DEF(autovendclose),
		ACMD_DEF2("avc", autovendclose),
