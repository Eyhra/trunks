...
bool pc_authok(struct map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	memset(&sd->cart_bkp, 0, sizeof(struct s_storage)); // [AutoVend]
...
// (^~_~^) LGP End
	...
	// Read the aura number enabled by the current character settings from the character's variable
	sd->ucd.aura.id = static_cast<int32>(pc_readglobalreg(sd, add_str(AURA_VARIABLE)));

	// If not a valid aura number, reset the associated variable and value to 0
	...
	intif_storage_request(sd,TABLE_CART_BKP, 0, STOR_MODE_ALL); // Request cart bkp data
 ...
 * ------------------------------------------------
 * Checa se a skill temporaria ja esta armazenada
 * caso não esteja adiciona ela, para assim poder
 * remove-la futuramente
 *-------------------------------------------------*/
void pc_check_temp_skill(struct map_session_data* sd, uint16 skill_id)
{
	...
	if (checkskill) {
		skillTemp skt{ skill_id };
		sd->Skills_Temp.push_back(skt);
	}
}
...
/*==========================================
 * Delete item on cart bkp for given index.
 *------------------------------------------*/
void pc_cart_bkp_delitem(struct map_session_data* sd, int n, int amount, int type, e_log_pick_type log_type)
{
	nullpo_retv(sd);

	if (sd->cart_bkp.u.items_cart_bkp[n].nameid == 0 ||
		sd->cart_bkp.u.items_cart_bkp[n].amount < amount)
		return;

	log_pick_pc(sd, log_type, -amount, &sd->cart_bkp.u.items_cart_bkp[n]);

	sd->cart_bkp.u.items_cart_bkp[n].amount -= amount;
	sd->cart_bkp_weight -= itemdb_weight(sd->cart_bkp.u.items_cart_bkp[n].nameid) * amount;
	if (sd->cart_bkp.u.items_cart_bkp[n].amount <= 0) {
		memset(&sd->cart_bkp.u.items_cart_bkp[n], 0, sizeof(sd->cart_bkp.u.items_cart_bkp[0]));
		sd->cart_bkp_num--;
	}
	if (!type) {
		clif_cart_delitem(sd, n, amount);
 ...
 * Return:
		...
		x = remaining itemid on cart after get
 *------------------------------------------*/
int pc_cart_bkp_item_amount(struct map_session_data* sd, int idx, int amount)
{
	struct item* item_data;

	nullpo_retr(-1, sd);

	item_data = &sd->cart_bkp.u.items_cart_bkp[idx];
	if (item_data->nameid == 0 || item_data->amount == 0)
		return -1;
...
static void pc_calcexp(struct map_session_data *sd, t_exp *base_exp, t_exp *job_exp, struct block_list *src)
{
	...
	if (*base_exp) {
		t_exp exp = (t_exp)(rateExp * (*base_exp + ((double)*base_exp * ((bonus + vip_bonus_base) / 100.))));
	...
	if (*job_exp) {
		t_exp exp = (t_exp)(rateExp * (*job_exp + ((double)*job_exp * ((bonus + vip_bonus_job) / 100.))));
 ...
 * Anti Mob PvP
 *------------------------------------------*/
bool pc_anti_mob(struct block_list* src, struct block_list* target)
{
	struct block_list* s_bl;
	struct map_session_data* sd, * pl_sd, * t_sd, * e_sd;
	...
	pl_sd = BL_CAST(BL_PC, s_bl);
	t_sd = BL_CAST(BL_PC, target);
	e_sd = BL_CAST(BL_PC, target);

	if (map_getmapflag(sd->bl.m, MF_ANTIMOB) || sd->state.antimob)
		if (sd->sc.data[SC_KSPROTECTED] && t_sd != sd)
			if ((e_sd = map_id2sd(sd->sc.data[SC_KSPROTECTED]->val1)) != NULL)
				if (e_sd != sd && e_sd != t_sd) {
					if (DIFF_TICK(sd->ks_floodprotect_tick, tick) <= 0) {
						sprintf(output, "[MeuRO] Anti-Mob! - Você esta lutando com [%s]. Aguarde para atacar outra pessoa.", e_sd->status.name);
						clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
	...
	if (map_getmapflag(sd->bl.m, MF_ANTIMOB) || sd->state.antimob)
		if (sd->bl.id != t_sd->bl.id)
			status_change_start(NULL, src, SC_KSPROTECTED, 10000, t_sd->bl.id, 1, 0, 0, battle_config.kspvpprotection, 0);

	if (!map_getmapflag(sd->bl.m, MF_ANTIMOB))
		if (!t_sd->state.antimob)
			return 0;

	do {
		if ((sce = sc->data[SC_KSPROTECTED]) == NULL)
			break;

		if (sd->bl.id == sce->val1)
			break;

		if ((pl_sd = map_id2sd(sce->val1)) == NULL)
			break;

		if (t_sd == pl_sd || t_sd == sd)
			break;

		// Message to KS
		if (DIFF_TICK(sd->ks_floodprotect_tick, tick) <= 0)
		{
			sprintf(output, "[MeuRO] Anti-Mob! - [%s] ja esta lutando com [%s]! Aguarde sua vez.", t_sd->status.name, pl_sd->status.name);
			clif_messagecolor(&sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);
		...
		// Message to Target
		if (DIFF_TICK(t_sd->ks_floodprotect_tick, tick) <= 0)
		{
			sprintf(output, "[MeuRO] Anti-Mob! [%s] esta tentando se juntar a batalha!", sd->status.name);
			clif_messagecolor(&t_sd->bl, color_table[COLOR_LIGHT_GREEN], output, false, SELF);

			t_sd->ks_floodprotect_tick = tick + 2000;
		}

		return 1;
	} while (0);

	status_change_start(NULL, target, SC_KSPROTECTED, 10000, sd->bl.id, 1, 0, 0, battle_config.kspvpprotection, 0);
...
bool pc_equipitem(struct map_session_data *sd,short n,int req_pos,bool equipswitch)
{
	...
	if( sd->sc.count && (sd->sc.data[SC_BERSERK] || sd->sc.data[SC_SATURDAYNIGHTFEVER] ||
		sd->sc.data[SC_KYOUGAKU] || ((sd->sc.data[SC_PYROCLASTIC] || sd->sc.data[SC_HNNOWEAPON]) && sd->inventory_data[n]->type == IT_WEAPON)) ) {
		if( equipswitch ){
			clif_equipswitch_add( sd, n, req_pos, ITEM_EQUIP_ACK_FAIL );
		}else{
			clif_equipitemack(sd,n,0,ITEM_EQUIP_ACK_FAIL); //Fail
	...
	if(pos & EQP_ARMOR && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipequipments.wav",0);	
	}
	if(pos & EQP_HEAD_TOP && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipequipments.wav",0);	
	}
	if(pos & EQP_HEAD_MID && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipequipments.wav",0);	
	}
	if(pos & EQP_HEAD_LOW && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipequipments.wav",0);	
	}
	if(pos & EQP_SHOES && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipequipments.wav",0);	
	}
	if(pos & EQP_GARMENT && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipequipments.wav",0);	
	}
	if(pos & EQP_ACC_L && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipequipments.wav",0);	
	}
	if(pos & EQP_ACC_R && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipequipments.wav",0);	
	}
	if(pos & EQP_HAND_R && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"equipweapon.wav",0);
    }
	if(pos & EQP_HAND_L && battle_config.button_sound_mode && sd->state.buttonsound == 1){
		if(id) {
			if(id->type == IT_WEAPON) {clif_soundeffect(sd,&sd->bl,"equipweapon.wav",0);}
			else if(id->type == IT_ARMOR) {clif_soundeffect(sd,&sd->bl,"equipshield.wav",0);}
				} 
	}
...
bool pc_unequipitem(struct map_session_data *sd, int n, int flag) {
	...
	if (!(flag&2) && sd->sc.count &&
		...
		((sd->sc.data[SC_PYROCLASTIC] || sd->sc.data[SC_HNNOWEAPON]) &&
	...
	if(pos & EQP_ARMOR && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"removeequipments.wav",0);	
	}
	if(pos & EQP_HEAD_TOP && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"removeequipments.wav",0);	
	}
	...
	if(pos & EQP_HAND_R && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"removeweapon.wav",0);
    }
	if(pos & EQP_HAND_L && battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"removeweapon.wav",0);
	}
 ...
 * @author [Shakto]
 **/
uint64 JobBonusDatabase::parseBodyNode( const YAML::Node &node ){
	uint32 id;
	...
	if (this->nodeExists(node, "Job")){
		...
		if(strcmp(job.c_str(), "All") == 0){
			...
			if (pcdb_checkid(jobid) && script_get_constant(job.c_str(),&jobid)) {
				//js->jobidx = jobid;
				js->jobidx = static_cast<int32>(jobid);
			}
			else {
	...
	if (this->nodeExists(node, "Script")) {
		...
		js->script = parse_script(script.c_str(), this->getCurrentFile().c_str(), node["Script"].Mark().line + 1, SCRIPT_IGNORE_EXTERNAL_BRACKETS);
	} else {
		if (!exists)
			js->script = nullptr;
	}
