...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if (bl->type == BL_PC) {
		...
		// antimob
		if (battle_config.kspvpprotection && pc_anti_mob(src, bl))
			return 0;
	...
	if (sc && sc->count) {
		...
		if(sc->data[SC_VIGOR]) {
			if( (flag&(BF_SHORT|BF_WEAPON)) == (BF_SHORT|BF_WEAPON) )
				damage += damage * sc->data[SC_VIGOR]->val1 / 20; // 50% increase to physical melee attacks
		}
	...
	if (bl->type == BL_PC) { // Increase damage for mobs
		mob_data* md = BL_CAST(BL_MOB, src);

		if (md && md->db->damageboost != 1000)
			damage = i64max(damage * md->db->damageboost * 1, 1);
	}
...
static int battle_range_type(struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	switch (skill_id) {
		...
		{// When using 2 Hand Spear = Long range, Using 2 hand sword = Short.
		...
		case RG_BACKSTAP: // 4 cell cast range.
		case AS_SONICBLOW: // 4 cell cast range.
...
static int battle_get_weapon_element(struct Damage* wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv, short weapon_position, bool calc_for_damage_only)
{
	...
	if( !skill_id || element == ELE_WEAPON ) {
		...
		if (sc && sc->data[SC_GRENADE_FRAGMENT] &&
			(skill_id == NW_BASIC_GRENADE || skill_id == NW_HASTY_FIRE_IN_THE_HOLE || skill_id == NW_GRENADES_DROPPING ||skill_id == NW_MISSION_BOMBARD))
		...
		case BS_HAMMERFALL:
			if (sd) {
					...
					if (index >= 0 && sd->inventory_data[index] && sd->inventory_data[index]->type == IT_WEAPON) {
						wd->damage = (int64)((sstatus->batk + ((sd->inventory_data[index]->weight / 10) * 3)) * (1.2 + (0.1 * (float)skill_lv)));
					}
				}
			break;
		case RK_DRAGONBREATH:
		case RK_DRAGONBREATH_WATER:
			{
				...
				if(sd)
					damagevalue = damagevalue * ((90 + 10 * pc_checkskill(sd, RK_DRAGONTRAINING)) + (pc_checkskill(sd, DK_DRAGONIC_AURA) * 10 + sstatus->patk + sstatus->pow)) / 100;
...
static int battle_calc_attack_skill_ratio(struct Damage* wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		case SS_SHIMIRU:
			skillratio += -100 + 700 * skill_lv + 5 * sstatus->pow;
			RE_LVL_DMOD(100);
			break;
		case SS_KAGEGARI:
			skillratio += 400 + 400 * skill_lv + 5 * sstatus->pow;
			if (sd)
		...
		case SS_KAGEAKUMU:
		case SS_HITOUAKUMU:
			skillratio += 14900 + 5 * sstatus->pow;
			if (tsc && tsc->data[SC_NIGHTMARE])
				skillratio += skillratio / 2;
			status_change_end(target, SC_NIGHTMARE, INVALID_TIMER);
			RE_LVL_DMOD(100);
			break;
		case SS_KAGENOMAI:
			skillratio += 400 + 600 * skill_lv + 5 * sstatus->pow;
			if (sd)
			...
			break;
		case SS_FUUMASHOUAKU:
			skillratio += 600 + 200 * skill_lv + 5 * sstatus->pow;
			if (sd)
				skillratio += pc_checkskill(sd, SS_FUUMAKOUCHIKU) * 5 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		case SS_FUUMAKOUCHIKU:
			skillratio += 500 + 400 * skill_lv + 5 * sstatus->pow;
			if (damage_flag)
			{
		...
		case SS_KUNAIWAIKYOKU:
			...
			if (sd)
				skillratio += pc_checkskill(sd, SS_KUNAIKUSSETSU) * 7 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		case SS_KUNAIKUSSETSU:
			skillratio += 100 + 360 * skill_lv + 5 * sstatus->pow;
			if (sd)
				skillratio += pc_checkskill(sd, SS_KUNAIKAITEN) * 7 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		case SS_KUNAIKAITEN:
			skillratio += 700 + 500 * skill_lv + 5 * sstatus->pow;
			if (sd)
				skillratio += pc_checkskill(sd, SS_KUNAIWAIKYOKU) * 7 * skill_lv;
			RE_LVL_DMOD(100);
			break;
		case SS_KAGEGISSEN:
			skillratio += 1600 + 800 * skill_lv + 5 * sstatus->pow;
			if (sd)
		...
		case SKE_STAR_BURST:
			...
			break;
		case SKE_DAWN_BREAK:
			skillratio += 200 + 400 * skill_lv + 5 * sstatus->pow;
		...
		case SKE_RISING_MOON:
			skillratio += 800 + 300 * skill_lv + 5 * sstatus->pow;
			if (sd && pc_checkskill(sd, SKE_SKY_MASTERY))
				skillratio += 5 * skill_lv * pc_checkskill(sd, SKE_SKY_MASTERY);
			RE_LVL_DMOD(100);
			break;
		case SKE_NOON_BLAST:
			skillratio += 1400 + 900 * skill_lv + 5 * sstatus->pow;
			if (sd && pc_checkskill(sd, SKE_SKY_MASTERY))
				skillratio += 5 * skill_lv * pc_checkskill(sd, SKE_SKY_MASTERY);
			RE_LVL_DMOD(100);
			break;
		case SKE_RISING_SUN:
			...
			break;
		case HN_SPIRAL_PIERCE_MAX:
			skillratio += 400 + 250 * skill_lv + 5 * sstatus->pow;
			if (sd && pc_checkskill(sd, HN_SELFSTUDY_TATICS))
			...
			break;
		case HN_SHIELD_CHAIN_RUSH:
			skillratio += 300 + 300 * skill_lv + 5 * sstatus->pow;
			if (sd && pc_checkskill(sd, HN_SELFSTUDY_TATICS))
				skillratio += 3 * skill_lv * pc_checkskill(sd, HN_SELFSTUDY_TATICS);
			RE_LVL_DMOD(100);
			break;
		case HN_MEGA_SONIC_BLOW:
			skillratio += 700+150 * skill_lv + 5 * sstatus->pow;
			if (sd && pc_checkskill(sd, HN_SELFSTUDY_TATICS))
			...
			break;
		case HN_DOUBLEBOWLINGBASH:
			skillratio += 200 * skill_lv + 5 * sstatus->pow;
			if (sd && pc_checkskill(sd, HN_SELFSTUDY_TATICS))
				skillratio += 3 * skill_lv * pc_checkskill(sd, HN_SELFSTUDY_TATICS);
			RE_LVL_DMOD(100);
			break;
		case SH_HOWLING_OF_CHUL_HO:
			skillratio += 400 + 750 * skill_lv + 5 * sstatus->pow;
			if (sd && pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY))
			...
			break;
		case SH_HOGOGONG_STRIKE:
			if (!(tsc && tsc->data[SC_HOGOGONG])) {
				skillratio *= 0;
				break;
			}
			skillratio += 100 + 100 * skill_lv + 5 * sstatus->pow;
			if (sd && pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY))
		...
		case MS_BOWLINGBASH:
			if (src && map_getcell(src->m, src->x, src->y, CELL_CHKLANDPROTECTOR)) {
				skillratio += 60 + 80 * skill_lv;
			} else {
				skillratio += 40 + 80 * skill_lv;
			}	
			break;
		case AS_GRIMTOOTH:
			skillratio += 20 * skill_lv;
...
//#endif
		...
		case MO_EXTREMITYFIST:
			if(sstatus->sp <= 6000)
				...
				skillratio = min(500000,skillratio); //Paramos em aproximadamente 50k SP para proteção contra estouro
			break;	
		...
		case RK_HUNDREDSPEAR:
			...
			if (sc) {
				...
				if (sc->data[SC_DRAGONIC_AURA])// Need official damage increase. [Rytech]
					skillratio += sc->data[SC_DRAGONIC_AURA]->val1 * 150;
			}
			break;
		case RK_WINDCUTTER:
			if (sd) {
		...
		case LG_CANNONSPEAR:
			skillratio += -100 + 120 * skill_lv;
			skillratio += sstatus->str * (1 * skill_lv);
			RE_LVL_DMOD(100);
			if (sc && sc->data[SC_SPEAR_SCAR])
				skillratio += skill_lv * 105;
			break;
		case LG_BANISHINGPOINT:
			...
			break;
		case LG_SHIELDPRESS:
			skillratio += -100 + 200 * skill_lv + sstatus->str + sstatus->vit;
			if (sd) {
				short index = sd->equip_index[EQI_HAND_L];
		...
		case LG_OVERBRAND:
			if(sc && sc->data[SC_OVERBRANDREADY])
				skillratio += -100 + 500 * skill_lv + (sstatus->dex + sstatus->str);
			else
				skillratio += -100 + 350 * skill_lv + (sstatus->dex + sstatus->str); // !TODO: What's the STR/VIT bonus?
		...
		case DK_SERVANT_W_DEMOL:
			...
			break;
		case DK_HACKANDSLASHER:
			skillratio += -100 + 500 + 250 * skill_lv + 5 * sstatus->pow;
		...
		case IQ_THIRD_FLAME_BOMB:
			skillratio += -100 + 650 * skill_lv + 5 * sstatus->pow;
			skillratio += status_get_max_hp(src) / 100 * 20;
			RE_LVL_DMOD(100);
			break;
		...
		case IG_SHIELD_SHOOTING:
			skillratio += -100 + 600 * skill_lv + 5 * sstatus->pow;
			skillratio += skill_lv * pc_checkskill(sd, IG_SHIELD_MASTERY) * 15 + 500;
		...
		case IG_OVERSLASH:
			...
			skillratio += skill_lv * pc_checkskill(sd, IG_SPEAR_SWORD_M) * 10;
			RE_LVL_DMOD(100);
			break;
		case CD_EFFLIGO:
...
static void battle_attack_sc_bonus(struct Damage* wd, struct block_list *src, struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sc) {
		...
		if((wd->flag&(BF_LONG|BF_MAGIC)) == BF_LONG) {
			...
			if (sc->data[SC_BO_HELL_DUSTY]) { //Hell Dusty Buff from Hell Tree Increase Ranged attack.
...
static void battle_calc_defense_reduction(struct Damage* wd, struct block_list *src,struct block_list *target, uint16 skill_id, uint16 skill_lv)
{
	...
	if (sc && sc->data[SC_A_TELUM]) {
		short i = 5 * sc->data[SC_A_TELUM]->val1; // 5% per level
...
//#ifdef RENEWAL
			...
			case IG_OVERSLASH:
				wd.div_ += wd.miscflag;
				if (wd.div_ > 7) // Number of hits doesn't appear to go above 7.
					wd.div_ = 7;
				break;
			case SHC_ETERNAL_SLASH:
...
struct Damage battle_calc_magic_attack(struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv,int mflag)
{
	...
	switch(skill_id) {
		...
		case SH_HYUN_ROK_CANNON:
			if (sc && sc->data[SC_COLORS_OF_HYUN_ROK])
			{
				switch (sc->data[SC_COLORS_OF_HYUN_ROK]->val1)
				{
				case 1:s_ele = ELE_WATER; break;
				case 2:s_ele = ELE_WIND; break;
				case 3:s_ele = ELE_EARTH; break;
				case 4:s_ele = ELE_FIRE; break;
				case 5:s_ele = ELE_DARK; break;
				case 6:s_ele = ELE_HOLY; break;
				}
			}
			break;
		case NPC_EARTHQUAKE:
...
#define MATK_ADD(a) { ad.damage += a; }
			...
			default: {
				...
				switch(skill_id) {
					...
					case SS_TOKEDASU:
						...
						break;
					case SS_SEKIENHOU:
						skillratio += 600 + 550 * skill_lv + 5 * sstatus->spl;
						if (sd && pc_checkskill(sd, SS_ANTENPOU))
							skillratio += 50 * pc_checkskill(sd, SS_ANTENPOU)*skill_lv;
						RE_LVL_DMOD(100);
						if (damage_flag)
							skillratio = skillratio * 3 / 10;
						break;
					case SS_REIKETSUHOU:
						...
						if (sd && pc_checkskill(sd, SS_ANTENPOU))
							skillratio += 25 * pc_checkskill(sd, SS_ANTENPOU)*skill_lv;
						RE_LVL_DMOD(100);
						break;
					case SS_KINRYUUHOU:
						skillratio += 50 + 220 * skill_lv + 5 * sstatus->spl;
						if (sd && pc_checkskill(sd, SS_ANTENPOU))
							skillratio += 7 * pc_checkskill(sd, SS_ANTENPOU)*skill_lv;
						RE_LVL_DMOD(100);
						break;
					case SS_ANKOKURYUUAKUMU:
						skillratio += -100 + 12000 * skill_lv + 5 * sstatus->spl;
						if (tsc && tsc->data[SC_NIGHTMARE])
							skillratio += skillratio / 2;
						status_change_end(target, SC_NIGHTMARE, INVALID_TIMER);
						RE_LVL_DMOD(100);
						break;
					case SS_RAIDENPOU:
						skillratio += -100 + 650 * skill_lv + 5 * sstatus->spl;
						if (sd && pc_checkskill(sd, SS_ANTENPOU))
							skillratio += 50 * pc_checkskill(sd, SS_ANTENPOU)*skill_lv;
						RE_LVL_DMOD(100);
						if (damage_flag)
						...
						break;
					case SS_ANTENPOU:
						skillratio += 300 + 500 * skill_lv + 5 * sstatus->spl;
						RE_LVL_DMOD(100);
						if (damage_flag)
							skillratio = skillratio * 3 / 10;
						break;
					case HN_NAPALM_VULCAN_STRIKE:
						skillratio += 150 + 250 * skill_lv + 5 * sstatus->spl;
						if (sd && pc_checkskill(sd, HN_SELFSTUDY_SOCERY))
							skillratio += 4 * pc_checkskill(sd, HN_SELFSTUDY_SOCERY);
						RE_LVL_DMOD(100);
						break;
					case HN_JUPITEL_THUNDER_STORM:
						skillratio += -100 + 1800 * skill_lv + 5 * sstatus->spl;
						if (sd && pc_checkskill(sd, HN_SELFSTUDY_SOCERY))
							skillratio += 3 * pc_checkskill(sd, HN_SELFSTUDY_SOCERY);
						RE_LVL_DMOD(100);
						break;
					case HN_HELLS_DRIVE:
						skillratio += 900 + 550 * skill_lv + 5 * sstatus->spl;
						if (sd && pc_checkskill(sd, HN_SELFSTUDY_SOCERY))
							skillratio += 4 * pc_checkskill(sd, HN_SELFSTUDY_SOCERY);
						RE_LVL_DMOD(100);
						break;
					case HN_GROUND_GRAVITATION:
						if (damage_flag)
						{
							skillratio += 2900 + 1500 * skill_lv + 5 * sstatus->spl;
							if (sd && pc_checkskill(sd, HN_SELFSTUDY_SOCERY))
								skillratio += 4 * pc_checkskill(sd, HN_SELFSTUDY_SOCERY);
						}
						else
						{
							skillratio += 300 + 300 * skill_lv + 5 * sstatus->spl;
							if (sd && pc_checkskill(sd, HN_SELFSTUDY_SOCERY))
								skillratio += 2 * pc_checkskill(sd, HN_SELFSTUDY_SOCERY);
						}
						RE_LVL_DMOD(100);
						break;
					case HN_JACK_FROST_NOVA:
						...
						break;
					case HN_METEOR_STORM_BUSTER:
						skillratio += 200 + 150 * skill_lv + 5 * sstatus->spl;
						if (damage_flag)
						{
							ad.div_ = -3;
						...
						if (sd && pc_checkskill(sd, HN_SELFSTUDY_SOCERY))
							skillratio += 5 * pc_checkskill(sd, HN_SELFSTUDY_SOCERY);
						RE_LVL_DMOD(100);
						break;
					case SH_HYUN_ROKS_BREEZE:
						skillratio += 400 + 250 * skill_lv + 5 * sstatus->spl;
						if (sd && pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY))
							skillratio += 20 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY);
						...
						break;
					case SH_HYUN_ROK_CANNON:
						skillratio += 500 + 400 * skill_lv + 5 * sstatus->spl;
						if (sd && pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY))
							skillratio += 50 * pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY);
						if (sd && pc_checkskill(sd, SH_COMMUNE_WITH_HYUN_ROK))
						{
							skillratio += 150 * skill_lv - 50;
							if (sd && pc_checkskill(sd, SH_MYSTICAL_CREATURE_MASTERY))
					...
					case SOA_EXORCISM_OF_MALICIOUS_SOUL:
						...
						if (map_find_skill_unit_oncell(src, src->x, src->y, SOA_TOTEM_OF_TUTELARY, nullptr, 0) != nullptr)
							skillratio += 100 * skill_lv;

						if (sd)
						{
							skillratio += (pc_checkskill(sd, SOA_SOUL_MASTERY)) * 2;
							skillratio *= sd->soulball;
						}
						RE_LVL_DMOD(100);
						break;
					case SOA_TALISMAN_OF_BLUE_DRAGON:
						...
						if (sd)
						{
							skillratio += (pc_checkskill(sd, SOA_TALISMAN_MASTERY)) * 15;
						}
						if (sc && sc->data[SC_TALISMAN_OF_FIVE_ELEMENTS] && sc->data[SC_T_FIVETH_GOD] )
							skillratio += 450 * skill_lv;
						RE_LVL_DMOD(100);
						break;
					...
					case SOA_TALISMAN_OF_RED_PHOENIX:
						skillratio += 1400 + 600 * skill_lv + 5 * sstatus->spl;
						if (sd)
						{
							skillratio += (pc_checkskill(sd, SOA_TALISMAN_MASTERY)) * 15;
						}
					...
					case SOA_TALISMAN_OF_BLACK_TORTOISE:
						skillratio += 1900 + 900 * skill_lv + 5 * sstatus->spl;
						if (sd)
						{
							skillratio += (pc_checkskill(sd, SOA_TALISMAN_MASTERY)) * 15;
						}
					...
					case SOA_CIRCLE_OF_DIRECTIONS_AND_ELEMENTALS:
						skillratio += -100 + 1500 * skill_lv + 5 * sstatus->spl;
						if (sd)
						{
							skillratio += (pc_checkskill(sd, SOA_TALISMAN_MASTERY) + pc_checkskill(sd, SOA_SOUL_MASTERY)) * 15;
						}
						RE_LVL_DMOD(100);
						if (sc && sc->data[SC_TALISMAN_OF_FIVE_ELEMENTS] && sc->data[SC_T_FIVETH_GOD])
							ad.div_ += 4;
					...
					case SOA_TALISMAN_OF_SOUL_STEALING:
						skillratio += 400 + 500 * skill_lv + 5 * sstatus->spl;
						if(sd)
							skillratio += (pc_checkskill(sd, SOA_TALISMAN_MASTERY) + pc_checkskill(sd, SOA_SOUL_MASTERY)) * 7;
						RE_LVL_DMOD(100);
						break;
					case WZ_STORMGUST:
#ifdef RENEWAL
					...
					case IG_CROSS_RAIN:
						skillratio += -100 + 150 * skill_lv + 5 * sstatus->spl;
						skillratio += skill_lv * 5 * pc_checkskill(sd, IG_SPEAR_SWORD_M);
						RE_LVL_DMOD(100);
		...
		if(!flag.imdef){
			...
			if (sc && sc->data[SC_A_VITA]) {
				i = 5 * sc->data[SC_A_VITA]->val1; // 5% per level
		...
		if (sc && sc->data[SC_SEVENWIND] && skill_id >= 5416) {
			int right_element = battle_get_weapon_element(&ad, src, target, 0, 1, EQI_HAND_R, false);
			ad.damage = battle_attr_fix(src, target, ad.damage, right_element, tstatus->def_ele, tstatus->ele_lv);
		}
...
int64 battle_calc_return_damage(struct block_list* bl, struct block_list *src, int64 *dmg, int flag, uint16 skill_id, bool status_reflect){
	...
	if (flag & BF_SHORT) {//Bounces back part of the damage.
		if (!status_reflect && skill_id == WS_CARTTERMINATION && !status_bl_has_mode(src, MD_STATUSIMMUNE)) //Ignoring all reflecting (psyz)
        {
            rdamage = 0;
            return 0;
        }
...
enum damage_lv battle_weapon_attack(struct block_list* src, struct block_list* target, t_tick tick, int flag) {
	...
	if (sc && sc->data[SC_AUTO_FIRING_LAUNCHER])
	{
		...
		if (sd && skill_id) {
			uint16 skill_lv = pc_checkskill(sd, skill_id);
			if (skill_check_condition_castend(sd, skill_id, skill_lv)) {
				skill_consume_requirement(sd, skill_id, skill_lv, 2);
				skill_castend_pos2(src, target->x, target->y, skill_id, pc_checkskill(sd, skill_id), tick, flag);
			}
		}
	}
	...
	if (sd) {
		...
		if( sc ){
			...
			if (sc && sc->data[SC_SERVANTWEAPON] && sd->servantball > 0 && rnd() % 100 < 15) {
			...
			// It has a success chance of triggering even tho the description says nothing about it.
			// TODO: Need to find out what the official success chance is. [Rytech]
			if (sc && sc->data[SC_ABYSSFORCEWEAPON] && sd->abyssball > 0 && rnd() % 100 < 15) {
			...
			// It has a success chance of triggering even tho the description says nothing about it.
			// TODO: Need to find out what the official success chance is. [Rytech]
			if (sc && sc->data[SC_ABYSSFORCEWEAPON] && rnd() % 100 < 15) {
...
// rAthena
	...
	{ "homunculus_S_max_level",             &battle_config.hom_S_max_level,                 120,    0,      MAX_LEVEL,      },
	...
	// [AutoVend]: Global Coins
	{ "item_zeny",							&battle_config.item_zeny,					7227,		0,		INT32_MAX,		},
	{ "item_cash",							&battle_config.item_cash,					674,		0,		INT32_MAX,		},
	...
	{ "autovend_all_lvl",					&battle_config.autovend_all_lvl,				5,      0,		99,				},
	...
	// antimob
	{ "kspvpprotection",                    &battle_config.kspvpprotection,                 5000,   0,      INT_MAX, },
	
	// Sons dos botoes
	{ "button_sound_mode",						&battle_config.button_sound_mode,					1,	0,	1,	}, // @security [Cydh]
