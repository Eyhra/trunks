...
// Since walking NPCs are not supported on official servers, the client does not know how to handle it.
					...
					return ( pcdb_checkid(status_get_viewdata(bl)->class_) && walking ) ? 0x0 : 0x6; // New walking NPC type
#else
...
// Description: Send bl's aura effect information to tsd client
// Access:      public 
...
// Description: Send sd's aura information to other clients in the specified range
// Access:      public 
// Parameter:   struct block_list * bl
// Parameter:   enum send_target target
// Parameter:   bool ignore_hidden Aura message is not sent if the character is hidden
...
/* Might be earlier, this is when the named item bug began */
	...
	// If the packet is sent to a player unit,
	// then draw the halo effect of bl here to the player client corresponding to tbl
	if (tbl->type == BL_PC) {
...
/* Might be earlier, this is when the named item bug began */
	...
	// When a unit is notified by clif_spawn_unit to a client to spawn,
	// also apply its corresponding aura to this unit.
	// clif_spawn_unit it is the underlying function of clif_spawn, and it should be handled more thoroughly
...
/* Might be earlier, this is when the named item bug began */
	...
	if (tsd) {
		// There are only two references to the entire clif_set_unit_walking, if tsd is not empty, it means
		// this is the call from clif_getariacar_unit
		// At this point, we can send the aura information of bl to the client of tsd and request to draw the aura
		clif_send_auras_single(bl, tsd);
	}
	else {
		// If tsd is empty then this is a call from clif_move2
		// The main function of this packet at this time is to force the client to redraw the entire unit unit, forcing the movement speed to take effect
		// Due to a design flaw in the client (202 / 362 is bound to a paper doll instead of a unit)
		// Using clif_set_unit_walking will force the paper doll to be redrawn, causing the 202 / 362 special effect to disappear
		// At this point, we add a packet here, asking the client to redraw these special effects
		//
		// Of course, the best solution is to not use clif_move2 in clif_move (clif_set_unit_walking is called at the end)
		// To force the client to update the movement speed of a unit. But I tried it now, and there is no particularly good method..
	...
	case BL_PC:
		{
			...
			// When the sd unit is generated, notify its client to load its own aura information
			// The reason why you don't need to notify others is because the top has already called the clif_spawn_unit function,
			// Inside the clif_spawn_unit, the aura information has been broadcasted to AREA_WOS (clients excluding itself in the field of view)
...
void clif_changeoption_target( struct block_list* bl, struct block_list* target ){
	...
	if (ucd && sc) {
		if (!ucd->aura.hidden && (status_ishiding(bl) || status_isinvisible(bl) || sc->data[SC_CAMOUFLAGE])) {
			// This unit is currently concealed and needs to hide the aura
			ucd->aura.hidden = true;

			// Notify other units within bl's field of view: bl has left the field of view
			// Force their clients to clear their cache on bl
			clif_clearunit_area(bl, CLR_OUTSIGHT);

			// Notify other BL_PC units within bl's field of view that bl has re-entered their field of view
			// Enable their clients to redraw bl units into the window
			...
			// This unit is not concealed and can restore aura
			ucd->aura.hidden = false;

			// Clients that send bl's aura information to other players around
			clif_send_auras(bl, AREA_WOS, false, AURA_SPECIAL_NOTHING);

			// If it is a player unit, then send the special effect of the body to itself (the wretched solution to the client defect)
			// Special aura effects like 202 / 362 will automatically disappear when reappearing after hiding or disguising
			// This is because in the client's internal logic, 202 / 362 is bound to the character paper doll
			// While other special effects are bound to the unit object, the unit does not disappear when hidden, but the paper doll is destroyed
			// actually re-creates a paper doll appearance when reverting to hide,
			// The new paper doll does not have 202 / 362 special effects, so here we need to re-issue a packet and redraw it
			if (bl->type == BL_PC) {
...
void clif_storageclose(struct map_session_data* sd)
{
	...
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"close.wav",0);
	}
}
...
void clif_skillinfoblock(struct map_session_data *sd)
{
	...
	/*====================================================================================
	 * Check Temporary Skills [NeoWolfer]
	 * -----------------------------------------------------------------------------------
	 * PCria um vector local, para armazenar as skills temporarias que ainda estão em uso
	 *------------------------------------------------------------------------------------*/
	std::vector<uint16> iSkills;
	...
	for ( i = 0, len = 4; i < MAX_SKILL; i++)
	{
		if( (id = sd->status.skill[i].id) != 0 )
		{
			...
			if(sd->status.skill[i].flag == SKILL_FLAG_PERMANENT)
				...
				{
				iSkills.push_back(id); // Armazena as Skills Temporarias que ainda estão ativas
				WFIFOB(fd, len + 36) = 0;
			}
			len += 37;
		}
	 ...
	 * Check Temporary Skills [NeoWolfer]
	 * ------------------------------------------------------------------
	 * Percorre a struct que armazenou as skills temporaria e verifica
	 * se ela ainda esta em uso comparando-a com a iSkills que foi
	 * armazenada um pouco mais acima
	 *-------------------------------------------------------------------*/
	for (auto it = sd->Skills_Temp.begin(); it < sd->Skills_Temp.end(); it++) {
		bool deleteskill = true;
		for (int j = 0; j < iSkills.size(); j++)
		{
			if (it->skill_id == iSkills[j]) {
				deleteskill = false;
				continue;
			}
		}
		...
		if (deleteskill) {
			clif_deleteskill(sd, it->skill_id);
			sd->Skills_Temp.erase(it);
			it--;
		}
	}
}
...
#endif
	// Solução por @Felipe FePa e @Rodrigo Dias
...
bool clif_skill_nodamage(struct block_list *src,struct block_list *dst, uint16 skill_id, int heal, t_tick tick)
{
	...
	if (sd && sd->class_&MAPID_NINJA && skill_id == TF_HIDING)
		return success;
...
void clif_openvendingreq(struct map_session_data* sd, int num)
{
	...
	// [AutoVend] Vending shouldn't open if nameid is 0 and extended vending is enabled
	// A venda não deve abrir se extended_vend.nameid for 0 e a venda estendida estiver habilitada
...
void clif_vendinglist( struct map_session_data* sd, struct map_session_data* vsd ){
	...
	for( int i = 0; i < vsd->vend_num; i++ ){
		...
		struct item_data* data = itemdb_search(vsd->status.autovend_check ? vsd->cart_bkp.u.items_cart_bkp[index].nameid : vsd->cart.u.items_cart[index].nameid );

		p->items[i].price = vsd->vending[i].value;
		p->items[i].amount = vsd->vending[i].amount;
		p->items[i].index = client_index( index );
		p->items[i].itemType = itemtype( vsd->status.autovend_check ? vsd->cart_bkp.u.items_cart_bkp[index].nameid : vsd->cart.u.items_cart[index].nameid );
		p->items[i].itemId = client_nameid( vsd->status.autovend_check ? vsd->cart_bkp.u.items_cart_bkp[index].nameid : vsd->cart.u.items_cart[index].nameid );
		p->items[i].identified = vsd->status.autovend_check ? vsd->cart_bkp.u.items_cart_bkp[index].identify : vsd->cart.u.items_cart[index].identify;
		p->items[i].damaged = vsd->status.autovend_check ? vsd->cart_bkp.u.items_cart_bkp[index].attribute : vsd->cart.u.items_cart[index].attribute;
		p->items[i].refine = vsd->status.autovend_check ? vsd->cart_bkp.u.items_cart_bkp[index].refine : vsd->cart.u.items_cart[index].refine;
		clif_addcards( &p->items[i].slot, vsd->status.autovend_check ? &vsd->cart_bkp.u.items_cart_bkp[index] : &vsd->cart.u.items_cart[index] );
#if PACKETVER >= 20150226
		clif_add_random_options( p->items[i].option_data, vsd->status.autovend_check ? &vsd->cart_bkp.u.items_cart_bkp[index] : &vsd->cart.u.items_cart[index] );
#if PACKETVER >= 20160921
		p->items[i].location = pc_equippoint_sub( sd, data );
		p->items[i].viewSprite = data->look;
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
		p->items[i].enchantgrade = vsd->status.autovend_check ? vsd->cart_bkp.u.items_cart_bkp[index].enchantgrade : vsd->cart.u.items_cart[index].enchantgrade;
#endif
#endif
#endif
	}
...
void clif_openvending( struct map_session_data* sd, int id, struct s_vending* vending ){
	...
	for( int i = 0; i < sd->vend_num; i++ ) {
		...
		p->items[i].itemType = itemtype(sd->status.autovend_check ? sd->cart_bkp.u.items_cart_bkp[index].nameid : sd->cart.u.items_cart[index].nameid);
		p->items[i].itemId = client_nameid(sd->status.autovend_check ? sd->cart_bkp.u.items_cart_bkp[index].nameid : sd->cart.u.items_cart[index].nameid);
		p->items[i].identified = sd->status.autovend_check ? sd->cart_bkp.u.items_cart_bkp[index].identify : sd->cart.u.items_cart[index].identify;
		p->items[i].damaged = sd->status.autovend_check ? sd->cart_bkp.u.items_cart_bkp[index].attribute : sd->cart.u.items_cart[index].attribute;
		p->items[i].refine = sd->status.autovend_check ? sd->cart_bkp.u.items_cart_bkp[index].refine : sd->cart.u.items_cart[index].refine;
		clif_addcards(&p->items[i].slot, sd->status.autovend_check ? &sd->cart_bkp.u.items_cart_bkp[index] : &sd->cart.u.items_cart[index]);
#if PACKETVER >= 20150226
		clif_add_random_options(p->items[i].option_data, sd->status.autovend_check ? &sd->cart_bkp.u.items_cart_bkp[index] : &sd->cart.u.items_cart[index]);
#if PACKETVER_MAIN_NUM >= 20200916 || PACKETVER_RE_NUM >= 20200724
		p->items[i].enchantgrade = sd->status.autovend_check ? sd->cart_bkp.u.items_cart_bkp[index].enchantgrade : sd->cart.u.items_cart[index].enchantgrade;
#endif
#endif
	}
...
void clif_refresh(struct map_session_data *sd)
{
	...
	// After refreshing with commands such as @refresh, you need to resend the aura information to the client
...
//			skill_unit_move(&sd->pd->bl,gettick(),1);
	...
	if(sd->state.connect_new) {
		...
		if (!sd->state.autotrade) { // Don't trigger NPC event or opening vending/buyingstore will be failed
			...
			status_change_end(&sd->bl, SC_VIP, INVALID_TIMER);
			if(sd->vip.time > 0)
				sc_start(NULL, &sd->bl, SC_VIP, 100, 1, (sd->vip.time-time(NULL)) * 1000);
#endif			
		}
		sd->autovend.active = false; // [AutoVend]
...
void clif_parse_WisMessage(int fd, struct map_session_data* sd)
{
	...
	if (dstsd == NULL || strcmp(dstsd->status.name, target) != 0) {
		...
		if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
		clif_soundeffect(sd,&sd->bl,"nonamepm.wav",0);
		}
		return;
	}
	...
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"pmnotification.wav",0);
	}
...
void clif_parse_NpcNextClicked(int fd,struct map_session_data *sd)
{
	...
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"npc_click.wav",0);
	}
}
...
void clif_parse_OpenVending(int fd, struct map_session_data* sd){
	...
	if (battle_config.extended_vending_enable && battle_config.extended_vending_showboard && sd->extended_vend.nameid) {
		struct item_data* item = itemdb_exists(sd->extended_vend.nameid);
...
void clif_parse_SkillSelectMenu(int fd, struct map_session_data *sd) {
	...
	if (sd->state.menu_or_input) {
		...
		if (sd->npc_idle_timer == INVALID_TIMER && !sd->state.ignoretimeout)
			return;
#endif
		int skill_id = RFIFOW(fd, info->pos[1]);
		sd->npc_amount = skill_id ? skill_id : -1;
		if (battle_config.idletime_option & IDLE_NPC_INPUT)
			sd->idletime = last_tick;
...
int clif_vend(struct map_session_data* sd, int skill_lv) {
	...
	for (i = 0; i < ARRAYLENGTH(item_vend); i++) {
		struct item_data* item;
		t_itemid nameid = item_vend[i].nameid;
