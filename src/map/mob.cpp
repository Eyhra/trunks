...
struct mob_data* mob_spawn_dataset(struct spawn_data *data)
{
	...
	md->damagetaken = -1; // By: KyoSasuke#5401
...
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, struct item_data* it)
{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			if(rateDrop && (it->type != IT_CARD || (mob->get_bosstype() != BOSSTYPE_MVP && mob->get_bosstype() != BOSSTYPE_MINIBOSS)))
				drop_rate += (int)(drop_rate * rateDrop / 100.);
		}
	}
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			if (md->dmgcount == 1) {	// HitKill
				drop_rate = min( (drop_rate * battle_config.hitkill_rate) / 100, 10000 );	// Cap it to 100%
				clif_specialeffect(&md->bl, 154, AREA);		// Efeito no mob
			}
			//ShowWarning("mob.cpp %d\n", drop_rate);
...
uint64 MobDatabase::parseBodyNode(const YAML::Node &node) {
	...
	if (this->nodeExists(node, "DamageBoost")) {
		uint16 damage;

		if (!this->asUInt16Rate(node, "DamageBoost", damage, 1000))
			return 0;

		mob->damageboost = damage;
	} else {
		if (!exists)
			mob->damageboost = 1000;
	}
...
static bool mob_read_sqldb_sub(std::vector<std::string> str) {
	...
	if (!str[++index].empty())
		node["DamageBoost"] = std::stoi(str[index]);
...
static int mob_read_sqldb(void)
{
	...
	for( uint8 fi = 0; fi < ARRAYLENGTH(mob_db_name); ++fi ) {
		...
		if( SQL_ERROR == Sql_Query(mmysql_handle, "SELECT `id`,`name_aegis`,`name_english`,`name_japanese`,`level`,`hp`,`sp`,`base_exp`,`job_exp`,`mvp_exp`,`attack`,`attack2`,`defense`,`magic_defense`,`str`,`agi`,`vit`,`int`,`dex`,`luk`,`attack_range`,`skill_range`,`chase_range`,`size`,`race`,"
			...
			"`element`,`element_level`,`walk_speed`,`attack_delay`,`attack_motion`,`damage_motion`,`damage_taken`,`damage_boost`,`ai`,`class`,"
