...
void trade_tradeack(struct map_session_data *sd, int type)
{
	...
	tsd->state.trading = 1;
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"oktrade.wav",0);
	}
...
void trade_tradeadditem(struct map_session_data *sd, short index, short amount)
{
	...
	if( trade_i == 10 ) { // No space left
		clif_tradeitemok(sd, index+2, 1);
		if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
		clif_soundeffect(sd,&sd->bl,"invalidtrade.wav",0);
		}
		return;
	}
	...
	if( sd->deal.item[trade_i].index == index ) { // The same item as before is being readjusted.
		...
		if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
		clif_soundeffect(sd,&sd->bl,"putitemtrade.wav",0);
		}
	} else { // New deal item
...
void trade_tradecommit(struct map_session_data *sd)
{
	...
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"completetrade.wav",0);
	}
