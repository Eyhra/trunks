...
static int npc_buylist_sub(struct map_session_data* sd, std::vector<s_npc_buy_list>& item_list, struct npc_data* nd) {
	...
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"buyitem.wav",0);
	}
...
static int npc_selllist_sub(struct map_session_data* sd, int n, unsigned short* item_list, struct npc_data* nd)
{
	...
	if(battle_config.button_sound_mode && sd->state.buttonsound == 1){
	clif_soundeffect(sd,&sd->bl,"sellitem.wav",0);
	}
	npc_event(sd, npc_ev, 0);
	return 0;
}
...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	switch( mapflag ){
		...
		// Skill Item Cost
		case MF_SKILL_ITEMCOST: {
			union u_mapflag_args args = {};

			if (!state)
				map_setmapflag_sub(m, MF_SKILL_ITEMCOST, false, &args);
			else {
				char skill_name[SKILL_NAME_LENGTH], item_name[3][16];

				if (sscanf(w4, "%30[^,],%15[^,],%15[^,],%15[^\n]", skill_name, item_name[0], item_name[1], item_name[2]) >= 2) {
					args.skill_itemcost.skill_id = skill_name2id(skill_name);

					if (!args.skill_itemcost.skill_id) {
						ShowError("npc_parse_mapflag: skill_itemcost: Invalid skill name '%s' for Skill ItemCost mapflag. Skipping (file '%s', line '%d')\n", skill_name, filepath, strline(buffer, start - buffer));
						break;
					}
					for (int i = 0; i < 3; i++) {
						if(itemdb_exists((args.skill_itemcost.item_id[i] = strtol(item_name[i], nullptr, 10))) == NULL) {
							args.skill_itemcost.item_id[i] = 0;
							//ShowWarning("npc_parse_mapflag: skill_itemcost: Invalid item ID '%d' for Skill ItemCost mapflag. Skipping (file '%s', line '%d')\n", args.skill_itemcost.item_id[i], filepath, strline(buffer, start - buffer));
							//break;
						}
						//ShowWarning("npc_parse_mapflag: skill_itemcost: skill name '%s' requires item ID '%d' for Skill ItemCost mapflag. Skipping ('%s', '%s', '%s')\n", skill_name, args.skill_itemcost.item_id[i], item_name[0], item_name[1], item_name[2]);
					}
					map_setmapflag_sub(m, MF_SKILL_ITEMCOST, true, &args);
				}
			}
			break;
		}
